...
use Documentos\Modelo\DAO\CgcDAO;
use Documentos\Formularios\CgcForm;
use Documentos\Modelo\Entidades\Cgc;

class CgcController extends AbstractActionController
{

    private $DAO;
    private $rutaLog = './public/log/';
    private $rutaArchivos = '/var/www/html/newportal/archivos/cgc/';
    /* private $rutaArchivos = './../archivos/cgc/'; */
    //------------------------------------------------------------------------------

    public function __construct(CgcDAO $dao)
    {
        $this->DAO = $dao;
    }
    ...
    //------------------------------------------------------------------------------
    function getFormulario($action = '', $idCGC = 0)
    {
        $form = new CgcForm($action);
        if ($idCGC != 0) {
            $rectoriaOBJ = $this->DAO->getCgc($idCGC);
    ...
    public function editarAction()
    {
        $request = $this->getRequest();
        if (!$request->isPost()) {
            $idCGC = (int) $this->params()->fromQuery('idCGC', 0);
            $infoCgc = $this->DAO->getCgcDetalle($idCGC);
            $form = new CgcForm('editar');
            $form->setData($infoCgc);
            $view = new ViewModel([
                'form' => $form,
            ]);
            $view->setTerminal(true);
    ...
    {
        $idCGC = (int) $this->params()->fromQuery('idCGC', 0);
        $infoEmpleado = $this->DAO->getCgcDetalle($idCGC);
    ...
    public function eliminarAction()
    {
        ...
        if (!$request->isPost()) {
            $idCGC = (int) $this->params()->fromQuery('idCGC', 0);
            $infoLven = $this->DAO->getCgcDetalle($idCGC);
            $form = new CgcForm('eliminar');
        ...
        if (!$form->isValid()) {
            ...
            return $this->redirect()->toUrl('index?idCGC=' . $this->params()->fromPost('idCGC', 0));
        }
        //----------------------------------------------------------------------
        try {
            $rectoriaOBJ->exchangeArray($form->getData());
        ...
        return $this->redirect()->toUrl('index?idCGC=' . $rectoriaOBJ->getIdCgc());
    }
    public function activarAction()
    {
        ...
        if (!$request->isPost()) {
            $idCGC = (int) $this->params()->fromQuery('idCGC', 0);
            $infoArchivo = $this->DAO->getCgcDetalle($idCGC);
            $form = new CgcForm('activar');
    ...
    {
        $idCGC = (int) $this->params()->fromRoute('id1', 0);
        $archivos = $this->DAO->getCgcDetalle($idCGC);
    ...
    {
        $idCGC = (int) $this->params()->fromQuery('idCGC', 0);
        $form = $this->getFormulario('actualizararchivo', $idCGC);
        $request = $this->getRequest();
        if ($request->isPost()) {
            $form->setData($request->getPost());
            if ($form->isValid()) {
                $session = $this->getInfoSesion();
                $rectoriaOBJ = new Cgc($form->getData());
                //----------------------------------------------------------------------
                $files = $request->getFiles()->toArray();
                //----------------------------------------------------------------------
                $uploadOK = new \Laminas\Validator\File\UploadFile();
