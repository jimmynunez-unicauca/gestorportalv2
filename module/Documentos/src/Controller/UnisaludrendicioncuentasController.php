...
use Documentos\Modelo\DAO\UnisaludrendicioncuentasDAO;
use Documentos\Formularios\UnisaludrendicioncuentasForm;
use Documentos\Modelo\Entidades\Unisaludrendicioncuentas;

class UnisaludrendicioncuentasController extends AbstractActionController
{

    private $DAO;
    private $rutaLog = './public/log/';
    private $rutaArchivos = '/var/www/html/newportal/archivos/unisalud/rendicion_cuentas/';
    //------------------------------------------------------------------------------

    public function __construct(UnisaludrendicioncuentasDAO $dao)
    {
        $this->DAO = $dao;
    }
    ...
    {
        $form = new UnisaludrendicioncuentasForm($action);
        if ($id != 0) {
            $usOBJ = $this->DAO->getUnisaludrendicioncuentas($id);
            $form->bind($usOBJ);
        }
        return $form;
    }
    ...
    public function registrarAction()
    {
        ...
        //----------------------------------------------------------------------
        $form = new UnisaludrendicioncuentasForm('registrar');
        ...
        //----------------------------------------------------------------------
        $usOBJ = new Unisaludrendicioncuentas();
        ...
        $respaldo = $this->procesarArchivo($files, 'ruta_archivo', ['pdf', 'docx', 'xlsx', 'pptx'], '250B', '25MB', 'RC', 'sinArchivo.pdf');
        ...
        try {
            ...
            $msgLog = "\n" . date('Y-m-d H:i:s') . " REGISTRAR ARCHIVO - UnisaludrendicioncuentasController->registrar \n"
                . $ex->getMessage()
                . "\n----------------------------------------------------------------------- \n";
    ...
    public function editarAction()
    {
        ...
        if (!$request->isPost()) {
            ...
            $form = new UnisaludrendicioncuentasForm('editar');
        ...
        //----------------------------------------------------------------------
        $form = new UnisaludrendicioncuentasForm('editar');
        $clOBJ = new Unisaludrendicioncuentas();
        ...
        try {
            ...
            $msgLog = "\n" . date('Y-m-d H:i:s') . " ACTUALIZAR CONTRATO LABORAL - UnisaludrendicioncuentasController->editar \n"
                . $ex->getMessage()
                . "\n----------------------------------------------------------------------- \n";
            $file = fopen($this->rutaLog . 'gestorportal.log', 'a');
            fwrite($file, $msgLog);
    ...
    public function eliminarAction()
    {
        ...
        if (!$request->isPost()) {
            $id = (int) $this->params()->fromQuery('id', 0);
            $infoArchivo = $this->DAO->getArchivoDetalle($id);
            $form = new UnisaludrendicioncuentasForm('eliminar');
            $form->setData($infoArchivo);
            $view = new ViewModel([
                'form' => $form,
            ]);
        ...
        try {
            ...
            $msgLog = "\n" . date('Y-m-d H:i:s') . " ELIMINAR ARCHIVO - UnisaludrendicioncuentasController->eliminar \n"
                . $ex->getMessage()
                . "\n----------------------------------------------------------------------- \n";
            $file = fopen($this->rutaLog . 'gestorportal.log', 'a');
            fwrite($file, $msgLog);
    ...
    public function activarAction()
    {
        ...
        if (!$request->isPost()) {
            ...
            $form = new UnisaludrendicioncuentasForm('activar');
        ...
        try {
            ...
            $msgLog = "\n" . date('Y-m-d H:i:s') . " ACTIVAR DOCUMENTO - UnisaludrendicioncuentasController->activar \n"
                . $ex->getMessage()
                . "\n----------------------------------------------------------------------- \n";
            $file = fopen($this->rutaLog . 'gestorportal.log', 'a');
            fwrite($file, $msgLog);
    ...
    public function desactivarAction()
    {
        ...
        if (!$request->isPost()) {
            ...
            $form = new UnisaludrendicioncuentasForm('desactivar');
            $form->setData($infoPluginunicauca);
        ...
        try {
            ...
            $msgLog = "\n" . date('Y-m-d H:i:s') . " DESACTIVAR DOCUMENTO - UnisaludrendicioncuentasController->desactivar \n"
                . $ex->getMessage()
                . "\n----------------------------------------------------------------------- \n";
            $file = fopen($this->rutaLog . 'gestorportal.log', 'a');
            fwrite($file, $msgLog);
    ...
    public function actualizararchivoAction()
    {
        ...
        if ($request->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                $usOBJ = new Unisaludrendicioncuentas($form->getData());
                //----------------------------------------------------------------------
                $files = $request->getFiles()->toArray();
                $respaldo = $this->procesarArchivo($files, 'ruta_archivo', ['pdf', 'docx', 'xlsx', 'pptx'], '250B', '25MB', 'RC', 'sinArchivo.pdf');
                if ($respaldo === false) return $this->redirect()->toUrl('index');
                ...
                try {
                    ...
                    $msgLog = "\n" . date('Y-m-d H:i:s') . " EDITAR Archivo - UnisaludrendicioncuentasController->registrar \n"
                        . $ex->getMessage()
                        . "\n----------------------------------------------------------------------- \n";
                    $file = fopen($this->rutaLog . 'GESTORPORTAL.log', 'a');
                    fwrite($file, $msgLog);
