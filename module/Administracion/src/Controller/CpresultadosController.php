...
use Administracion\Modelo\DAO\CpresultadosDAO;
use Administracion\Formularios\CpresultadosForm;
use Administracion\Formularios\ProcesoForm;
use Administracion\Modelo\Entidades\Cpresultados;

class CpresultadosController extends AbstractActionController
{

    private $DAO;
    private $rutaLog = './public/log/';
    private $rutaArchivos = '/var/www/html/newportal/archivos/centro_posgrados/resultados/';
    /* private $rutaArchivos = './../archivos/centro_posgrados/resultados/'; */
    //------------------------------------------------------------------------------

    public function __construct(CpresultadosDAO $dao)
    {
        $this->DAO = $dao;
    }
    ...
    //------------------------------------------------------------------------------
    function getFormulario($action = '', $idCPResultados = 0)
    {
        $form = new CpresultadosForm($action);
        if ($idCPResultados != 0) {
            $lvmenOBJ = $this->DAO->getCpresultados($idCPResultados);
            $form->bind($lvmenOBJ);
        }
        return $form;
    }
    ...
    public function registrarAction()
    {
        ...
        //----------------------------------------------------------------------
        $form = new CpresultadosForm('registrar', $listaDepenedencias);
    ...
    public function editarAction()
    {
        $request = $this->getRequest();
        if (!$request->isPost()) {
            $idCPResultados = (int) $this->params()->fromQuery('idCPResultados', 0);
            $infoCpresultados = $this->DAO->getCpresultadosDetalle($idCPResultados);
            $depenedencias = $this->DAO->getDependencias();
            $listaDepenedencias = array();
            foreach ($depenedencias  as $dep) {
                $listaDepenedencias[$dep['idDependencia']] = $dep['dependencia'];
            }
            $form = new CpresultadosForm('editar', $listaDepenedencias);
            $form->setData($infoCpresultados);
            $view = new ViewModel([
                'form' => $form,
            ]);
            $view->setTerminal(true);
        ...
        //----------------------------------------------------------------------
        $form = new CpresultadosForm('editar');
        $clOBJ = new Cpresultados();
    ...
    {
        $idCPResultados = (int) $this->params()->fromQuery('idCPResultados', 0);
        $infoEmpleado = $this->DAO->getCpresultadosDetalle($idCPResultados);
    ...
    public function eliminarAction()
    {
        ...
        if (!$request->isPost()) {
            $idCPResultados = (int) $this->params()->fromQuery('idCPResultados', 0);
            $infoLven = $this->DAO->getCpresultadosDetalle($idCPResultados);
            $depenedencias = $this->DAO->getDependencias();
            $listaDepenedencias = array();
            foreach ($depenedencias  as $dep) {
                $listaDepenedencias[$dep['idDependencia']] = $dep['dependencia'];
            }
            $form = new CpresultadosForm('eliminar', $listaDepenedencias);
        ...
        //----------------------------------------------------------------------
        $form = new CpresultadosForm('eliminar');
        $lvmenOBJ = new Cpresultados();
        $form->setInputFilter($lvmenOBJ->getInputFilter());
        $form->setData($request->getPost());
        if (!$form->isValid()) {
            ...
            return $this->redirect()->toUrl('index?idCPResultados=' . $this->params()->fromPost('idCPResultados', 0));
        }
        //----------------------------------------------------------------------
        try {
            $lvmenOBJ->exchangeArray($form->getData());
        ...
        return $this->redirect()->toUrl('index?idCPResultados=' . $lvmenOBJ->getIdCpresultados());
    }
    public function activarAction()
    {
        ...
        if (!$request->isPost()) {
            $idCPResultados = (int) $this->params()->fromQuery('idCPResultados', 0);
            $infoArchivo = $this->DAO->getCpresultadosDetalle($idCPResultados);
            $depenedencias = $this->DAO->getDependencias();
            $listaDepenedencias = array();
            foreach ($depenedencias  as $dep) {
                $listaDepenedencias[$dep['idDependencia']] = $dep['dependencia'];
            }
            $form = new CpresultadosForm('activar', $listaDepenedencias);
        ...
        //----------------------------------------------------------------------
        $form = new CpresultadosForm('activar');
        $lvmenOBJ = new Cpresultados();
        $form->setInputFilter($lvmenOBJ->getInputFilter());
        $form->setData($request->getPost());
        if (!$form->isValid()) {
    ...
    {
        $idCPResultados = (int) $this->params()->fromRoute('id1', 0);
        echo '<h1>TRIPLE HP</h1>';
        $archivos = $this->DAO->getCpresultadosDetalle($idCPResultados);
    ...
    {
        $idCPResultados = (int) $this->params()->fromQuery('idCPResultados', 0);
        $form = $this->getFormulario('actualizararchivo', $idCPResultados);
        $request = $this->getRequest();
        if ($request->isPost()) {
            $form->setData($request->getPost());
            if ($form->isValid()) {
                $session = $this->getInfoSesion();
                $lvmenOBJ = new Cpresultados($form->getData());
                //----------------------------------------------------------------------
                $files = $request->getFiles()->toArray();
                //----------------------------------------------------------------------
                $uploadOK = new \Laminas\Validator\File\UploadFile();
